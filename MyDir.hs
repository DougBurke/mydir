...
--
sortDC (DirContents ds ls xs fs) = DirContents
                                   (L.sort ds)
                                   (L.sort ls)
                                   (L.sort xs)
                                   (L.sort fs)
...
-- This step also removes "." and ".." from directories list
--                
lsToDir :: [(FilePath, FileType)] -> DirContents
lsToDir = sortDC . foldr conv emptyDir
    where
...
-- 
-- The current approach is not ideal, and doesn't work from here
-- since need to set up stdin for /bin/stty properly. Argh.
--
{-
terminalWidth :: IO Int
terminalWidth = do
  (ecode,out,err) <- readProcessWithExitCode "/bin/stty" ["size"] []
  putStrLn $ "ECODE == " ++ show ecode
  putStrLn $ "OUT == " ++ out
  putStrLn $ "ERR == " ++ err
  if ecode == ExitSuccess
    then (return . read . tail . dropWhile (/=' ')) out
    else return 80
-}

-- based on readProcess
terminalWidth :: IO Int
terminalWidth = do
  (_, Just outh, _, pid) <-
...
listContents :: Int -- ^ terminal width in characters
             -> FilePath -- ^ directory to lost
             -> IO ()
listContents tw dname = do
  cnts <- ls dname
  case cnts of
    Left eval   -> putStrLn ("Unable to access directory <" ++ dname ++ ">:\n" ++ show eval) >> exitFailure
    Right cnts' -> do
              let dcnts = lsToDir cnts'
                  ncols = 4
